...
# def count_insects(image_bytes: bytes, use_sam2_verification: bool = False) -> tuple[int, bytes, dict]:
#     """
#     Count insects with optional SAM 2 verification
...
#     result = results[0]
#     boxes = result.boxes
#     yolo_count = int(boxes.shape[0]) if boxes is not None else 0
    
#     metadata = {"yolo_count": yolo_count, "verified": False}
    
#     # SAM 2 verification if requested
#     if use_sam2_verification and yolo_count > 0:
#         try:
#             # Extract box coordinates for SAM 2
...
#             metadata["sam2_count"] = verification_results['verified_count']
#             metadata["confidence"] = verification_results['avg_confidence']
...
#             metadata["good_detections"] = len(verification_results['good_detections'])
#             metadata["bad_detections"] = len(verification_results['bad_detections'])
            
#             # Build detailed verification note
#             notes = []
#             notes.append(f"SAM 2 verified: {verification_results['verified_count']}/{yolo_count} high-quality")
#             notes.append(f"Quality: {verification_results['annotation_quality']}")
            
#             if verification_results['quality_issues']:
#                 notes.append(f"{len(verification_results['quality_issues'])} potential issues found")
            
#             metadata["verification_note"] = " | ".join(notes)
            
#             # Use verified count
#             count = verification_results['verified_count']
#         except Exception as e:
#             # Fall back to YOLO if SAM 2 fails
#             count = yolo_count
...
#     return count, buffer.getvalue(), metadata
...
#             help="Activate SAM 2 to double-check counts when approaching alert threshold. Recommended when ratio is 12-16 insects per 100 images.",
#             key="sam2_toggle",
...
#             st.info("🔬 SAM 2 verification active: Each detection will be validated with instance segmentation for higher accuracy.")
...
#                     insect_count, annotated, metadata = count_insects(bytes_data, use_sam2_verification=use_sam2)
#                     cache[file.name] = {
...
#                         'Verified Count': metadata.get('sam2_count', 0),
...
#                     st.write(f"- SAM 2 verified: {metadata.get('sam2_count', 0)} high-quality")
#                     st.write(f"- Annotation quality: **{metadata.get('annotation_quality', 'Unknown')}**")
#                     st.write(f"- Average confidence: {metadata.get('confidence', 0):.2f}")
...
# import sys
# sys.path.insert(0, "/Users/saurav/Documents/stinkbug/Stinkbug-Project/GroundingDINO")
...
# Fix: Use Path.cwd() for BASE_DIR as __file__ is not always defined in notebooks
BASE_DIR = Path.cwd()
load_dotenv(dotenv_path=BASE_DIR / ".env", override=False)
...
Action: [tool_name with arguments]"""
        ...
        # --- Improved: Parallel execution for independent tool calls ---
        # Identify independent tool calls (no dependencies, no repeated tool names)
        parallel_candidates = []
        sequential_steps = []
        used_tools = set()
...
Your turn:"""
    ...
    def synthesize_answer(self, user_query: str) -> Dict:
        ...
        # (Removed entity comparison/missing data fallback logic. The agent will always provide a full answer.)

        # --- End new logic ---

        # (rest of the original method follows...)
        # Check if this includes trend data for better formatting
...
# ============================================================================
# END ADVANCED AGENTIC LLM SYSTEM
# ============================================================================
        ...
        if status["available"]:
            try:
                ...
                if text_boxes:
                    ...
                    # No matching regions found - use fallback (keep YOLO boxes)
                    # GroundingDINO struggles with small objects, so 0 detections could mean:
                    # 1) Objects too small to detect (keep YOLO boxes - correct insects)
                    # 2) Wrong objects detected by YOLO (should filter out - leaves)
                    # Conservative approach: keep YOLO boxes to avoid missing valid detections
    ...
    if df.empty:
        ...
        with st.expander("🐛 Troubleshooting"):
            ...
            **If buttons don't work:**
            ...
            6. Check terminal output for ✅ or ❌ messages
